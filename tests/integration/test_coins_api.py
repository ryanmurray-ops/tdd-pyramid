def test_get_coins_returns_200(client):
    response = client.get("/coins")
    assert response.status_code == 200

def test_get_coins_returns_empty_list(client):
    response = client.get("/coins")
    assert response.status_code == 200
    assert response.json == []

def test_get_all_coins_returns_created_coins(app, client):
...
def test_get_coin_by_id_returns_correct_coin(app, client):
...
def test_get_coin_by_id_returns_404_and_error_message_when_not_found(client):
    response = client.get("/coins/non-existent-id")
    assert response.status_code == 404
    assert response.json["error"] == "Coin not found"

def test_create_coin_via_api_returns_201_and_coin(client):
...
def test_create_coin_returns_400_when_name_already_exists(client):
    client.post("/coins", json={"name": "Automate"})
    response = client.post("/coins", json={"name": "Automate"})
    assert response.status_code == 400

def test_create_coin_returns_error_message_when_name_already_exists(client):
    client.post("/coins", json={"name": "Automate"})
    response = client.post("/coins", json={"name": "Automate"})
    assert response.status_code == 400
    assert response.json["error"] == "Coin already exists"

def test_put_coin_endpoint_returns_200(app, client):
    coin = app.coin_service.create_coin("Automate")
    response = client.put(
        f"/coins/{coin.id}",
...
def test_put_unknown_coin_returns_404(client):
    response = client.put(
        "/coins/{coin.id}",
        json={"is_complete": True}
    )
...
def test_put_unknown_coin_returns_error_message(client):
    response = client.put(
        "/coins/non-existent-id",
        json={"is_complete": True}
    )
    coin_update_request = response.get_json(client)
    assert response.status_code == 404
    assert coin_update_request["error"] == "Coin not found"

def test_delete_coin_endpoint_returns_200(app, client):
    coin = app.coin_service.create_coin("Automate")
    response = client.delete(f"/coins/{coin.id}")  
    assert response.status_code == 200

def test_delete_coin_removes_coin_from_service(app, client):
    coin = app.coin_service.create_coin("Automate")
    client.delete(f"/coins/{coin.id}")
    result = app.coin_service.get_coin_by_id(coin.id)
    assert result is None

def test_delete_unknown_coin_returns_404(client):
    response = client.delete("/coins/non-existent-id")
    assert response.status_code == 404

def test_delete_unknown_coin_returns_error_message(client):
    response = client.delete("/coins/non-existent-id")
